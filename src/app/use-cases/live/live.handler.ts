...
import { LiveEvent, LiveEventData } from "../../../infra/f1-client/types/live-events.types.js";
...
export class LiveHandlerFactory {
  private readonly handlers: {
    [Topic.TIMING_DATA]: (data: LiveEventData[Topic.TIMING_DATA]) => Promise<void>;
    [Topic.SESSION_INFO]: (data: LiveEventData[Topic.SESSION_INFO]) => Promise<void>;
    [Topic.DRIVER_LIST]: (data: LiveEventData[Topic.DRIVER_LIST]) => Promise<void>;
  };
  constructor(
    ...
    this.handlers = {
      ...
      [Topic.SESSION_INFO]: handleSessionInfo(this.cache, this.sessionRepository),
  ...
  async handleLiveUpdates<TTopic extends Topic = Topic>(
    [topic, data, _timestamp]: LiveEvent<TTopic>
  ) {
